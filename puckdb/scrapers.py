...
async def fetch(query_by: query.BaseQuery, concurrency=5,
                loop: asyncio.AbstractEventLoop = asyncio.get_event_loop(), save: bool = True):
    sem = asyncio.Semaphore(concurrency, loop=loop)

async def _wait_progress(tasks, loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()) -> Iterable[asyncio.Future]:
    for f in tqdm.tqdm(asyncio.as_completed(tasks, loop=loop), total=len(tasks)):
        await f
...
class BaseScraper(object):
    ...
    @abc.abstractmethod
    def __init__(self, query_by: query.BaseQuery = None, concurrency: int = 5,
    ...
    async def _fetch(self, session: aiohttp.ClientSession, url: str) -> List[dict]:
        async with self.sem:
            async with session.get(url, headers=query.HEADERS) as response:
...
class TeamScraper(BaseScraper):
    ...
    def __init__(self, query_by: query.BaseQuery = None, concurrency: int = 5,
                 loop: asyncio.AbstractEventLoop = None, save: bool = True):
        super().__init__(query_by, concurrency, loop, save)
...
class ScheduleScraper(BaseScraper):
    ...
    def __init__(self, query_by: query.GameQuery, concurrency: int = 5, loop: asyncio.AbstractEventLoop = None,
                 save: bool = True):
        super().__init__(query_by, concurrency, loop, save)
    ...
    def _get_tasks(self, session: aiohttp.ClientSession) -> List[asyncio.Future]:
        urls = [
            ...
            for interval in self.query_by.intervals]
        return [asyncio.ensure_future(self._fetch(session, url), loop=self.loop) for url in urls]
...
class GameScraper(BaseScraper):
    ...
    def __init__(self, query_by: query.GameQuery, concurrency: int = 3, loop: asyncio.AbstractEventLoop = None,
                 save: bool = True):
        super().__init__(query_by, concurrency, loop, save)
    ...
    def _get_tasks(self, session: aiohttp.ClientSession) -> List[asyncio.Future]:
        schedule_games = ScheduleScraper(self.query_by, loop=self.loop).get()
